...
btn.addEventListener(`click`, (e) => {
  e.preventDefault();
  let paragraphs = '';
  let numOfParagraphs = +amount.value;
  if (numOfParagraphs > 0)
    while (numOfParagraphs--) paragraphs += `<p class="result">${text[Math.floor(Math.random() * text.length)]}</p>`;
  article.innerHTML = paragraphs || `<p class="result">Type a correct value</p>`;
});